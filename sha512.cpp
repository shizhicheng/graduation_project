...
/**
 * 一轮加密过程
 * message 为1024位的明文块  block_nb为明文分组的数目
 */
void SHA512::transform(const unsigned char *message, unsigned int block_nb) {
    ...
    for (i = 0; i < (int) block_nb; i++) {
        //该运算相当于每次移128个字符，即1024位，即下一个分组的位置
 ...
 *
 * @param message 明文段
 * @param len 明文长度（单位：字节）
 * SHA512_BLOCK_SIZE为明文分组长度128字节
 * 对前n-1个分组进行散列
 * 前n-1个分组len的长度为1024
 */
void SHA512::update(const unsigned char *message, unsigned int len) {
    unsigned int block_nb;
    unsigned int new_len = 0, rem_len = 0, tmp_len = 0;
    const unsigned char *shifted_message = NULL;
    tmp_len = SHA512_BLOCK_SIZE - m_len;
    rem_len = len < tmp_len ? len : tmp_len;//选取最小的数
...
void SHA512::final(unsigned char *digest) {
    ...
    len_b = (m_tot_len + m_len) << 3;//明文的总长度，由char换算成bit
    pm_len = block_nb << 7;//最后一个短分组经过填充之后的总长度
    memset(m_block + m_len, 0, pm_len - m_len);//对短分组记性填充
    m_block[m_len] = 0x80;
    SHA2_UNPACK32(len_b, m_block + pm_len - 4);//填充明文长度
    transform(m_block, block_nb);//对该分组进行散列
    for (i = 0; i < 8; i++) {//更新寄存器中的值
